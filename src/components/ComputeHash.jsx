// Take a string as an argument and return a Hex string of 64 multiples bytes
const padding = (M) => {
    let sizeLastBlock = 64; // Block size at the end of M
    const sizeMLengthBuffer = 8; // The end 8bytes of the final message describes the size of M
    const sizeDivision = 1; // Message bytes and remainder of 0 bytes separated (0x80)
    const sizeMaxBlock = sizeLastBlock - sizeMLengthBuffer - sizeDivision; // The message byte may be less than or equal to 55bytes.
    const sizeM = encodeURI(M).replace(/%../g, "*").length;
    const sizeLastM = sizeM % 64;
    const isOverflow = sizeMaxBlock < sizeLastM;

    if (isOverflow)
        sizeLastBlock = 128; // Add 1 block because the byte size of the last block of M exceeds 55bytes and cannot be stored in 1 block
    
    // Convert M to hex hex string
    const hexStringM = Array.from((new TextEncoder()).encode(M)).map(v => v.toString(16)).join("");

    // Generate arrays of too many 0s
    const sizeExtra = (sizeLastBlock - sizeMLengthBuffer - sizeDivision - sizeLastM) * 2;
    const hexExtraString = Array(sizeExtra).fill(0).join("");

    // Represent the size of M in 8bytes
    const hexSizeM = (sizeM * 8).toString(16);
    const hex8bytesLengthM = Array(16 - hexSizeM.length).fill(0).concat(hexSizeM).join("");

    // Generate PaddedString multiples of 64bytes
    const resultString = hexStringM + "80" + hexExtraString + hex8bytesLengthM;

    return resultString;
};

// Returns an array divided by every 64bytes with the Hex string of 64 multiple bytes as an argument
const divideM = (M) => {
    // 64bytes is 128 characters since it is treated as a string
...
// a constant of 4bytes of decimal points in the cubic roots of the smallest to 64 primes
const K = [
...
// Main function
const ComputeHash = (M) => {
    //Arrays that contain hash values per block
    ...
    for (let i = 0; i < dividedM.length; i++) {
        const Mi = dividedM[i].match(/.{8}/g); // Engrave more 64bytes blocks by 4byte
